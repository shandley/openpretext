...
Second, the European lancelet haplotype 1 (tau = 0.903) represents the algorithm's weakest performance. Investigation reveals that at the 1,024-pixel overview resolution used for link scoring, only 19 of 65 contigs occupy the minimum 4 pixels required for reliable score computation; the remaining 46 contigs are effectively invisible to the algorithm. Because *Branchiostoma lanceolatum* has 19 chromosomes of relatively uniform size --- a karyotype unusual among chordates --- this results in approximately one scorable contig per chromosome. With so few scorable contigs, inter-chromosome link scores become comparable to intra-chromosome scores, causing the Union Find chaining to merge contigs across chromosome boundaries. This is a fundamental resolution limit of operating on a downsampled overview: all 10 CI regression specimens have fewer than 60 scorable contigs (range 8--52), regardless of total contig count (range 52--5,506). The algorithm mitigates this with a low-contig guard (returning trivial chains for assemblies with fewer than 60 total contigs), but the lancelet's 65 contigs fall just above this threshold. Loading at higher overview resolution (4,096 pixels, mipmap level 3) was tested but does not improve results: the lancelet's tiny scaffolds (1--6 texture pixels each) remain sub-pixel even at 4x resolution, and the algorithm's thresholds, tuned for 1,024-pixel signal characteristics, produce slightly worse scores on other specimens at the higher resolution. Running AutoSort on all 10 pre-curation assemblies confirms that the algorithm produces meaningful multi-contig chains only for the chromosome-scale contigs (1--13 chains per specimen versus 13--32 known chromosomes), acting as a starting point that groups the largest contigs before manual curation assigns the smaller fragments.
