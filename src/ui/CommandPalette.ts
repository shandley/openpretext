...
function getCommands(ctx: AppContext) {
  ...
  return [
    ...
    { name: 'Auto sort: Union Find', shortcut: '\u2325S', action: () => runAutoSort(ctx) },
    { name: 'Auto cut: detect breakpoints', shortcut: '\u2325C', action: () => runAutoCut(ctx) },
    ...
    { name: 'Tutorial: 3D Genomics Analysis', shortcut: '', action: () => ctx.tutorialManager?.startLesson(ctx, '07-analysis-workflow') },
    { name: 'Tutorial: Classifying Contigs with Meta Tags', shortcut: '', action: () => ctx.tutorialManager?.startLesson(ctx, '08-contig-classification') },
    { name: 'Tutorial: Automated Misassembly Detection', shortcut: '', action: () => ctx.tutorialManager?.startLesson(ctx, '09-automated-misassembly') },
