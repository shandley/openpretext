...
describe('AGPWriter', () => {
  ...
  describe('AGP export determinism', () => {
    // Curation-as-code only means anything if the same script gives the same
    // file. A wall-clock stamp in the header broke that: two runs of one script
    // on one assembly differed in exactly that line and nowhere else.
    it('produces byte-identical output for the same state', () => {
      const contigs = [makeContig('ctg1', 0, 1000, 1), makeContig('ctg2', 1, 2000, 1)];
      const state = makeAppState(contigs, [0, 1]);
      expect(exportAGP(state)).toBe(exportAGP(state));
    });

    it('stamps no timestamp by default', () => {
      const contigs = [makeContig('ctg1', 0, 1000, 1)];
      expect(exportAGP(makeAppState(contigs, [0]))).not.toContain('Generated by OpenPretext');
    });

    it('stamps one when asked, so provenance is still available', () => {
      const contigs = [makeContig('ctg1', 0, 1000, 1)];
      const out = exportAGP(makeAppState(contigs, [0]), { timestamp: '2026-07-31T00:00:00.000Z' });
      expect(out).toContain('# Generated by OpenPretext on 2026-07-31T00:00:00.000Z');
    });

    it('keeps the version line whether or not a timestamp is stamped', () => {
      expect(generateAGPHeader(null)).toContain('##agp-version\t2.1');
      expect(generateAGPHeader(null)).not.toContain('Generated by');
      expect(generateAGPHeader('2026-01-01T00:00:00.000Z')).toContain('##agp-version\t2.1');
    });
  });
